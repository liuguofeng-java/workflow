<template>
  <div class="toolbar-container">
    <div class="left-toolbar">
      <el-button link type="primary" :disabled="undoDisabled" :title="i18nt('designer.toolbar.undoHint')" @click="undoHistory"> <svg-icon icon-class="undo" /></el-button>
      <el-button link type="primary" :disabled="redoDisabled" :title="i18nt('designer.toolbar.redoHint')" @click="redoHistory"> <svg-icon icon-class="redo" /></el-button>
      <el-button-group style="margin-left: 20px" v-show="false">
        <el-button :type="layoutType === 'PC' ? 'info' : ''" @click="changeLayoutType('PC')"> {{ i18nt("designer.toolbar.pcLayout") }}</el-button>
        <el-button :type="layoutType === 'Pad' ? 'info' : ''" @click="changeLayoutType('Pad')"> {{ i18nt("designer.toolbar.padLayout") }}</el-button>
        <el-button :type="layoutType === 'H5' ? 'info' : ''" @click="changeLayoutType('H5')"> {{ i18nt("designer.toolbar.mobileLayout") }}</el-button>
      </el-button-group>
      <el-button style="margin-left: 20px" :title="i18nt('designer.toolbar.nodeTreeHint')" @click="showNodeTreeDrawer"> <svg-icon icon-class="node-tree" /></el-button>
    </div>
    ...
    <el-drawer :title="i18nt('designer.toolbar.nodeTreeTitle')" direction="ltr" v-model="showNodeTreeDrawerFlag" :modal="true" :size="280" :destroy-on-close="true" class="node-tree-drawer">
      <el-tree ref="nodeTree" :data="nodeTreeData" node-key="id" default-expand-all highlight-current class="node-tree" icon-class="el-icon-arrow-right" @node-click="onNodeTreeClick"></el-tree>
    </el-drawer>

    <div class="right-toolbar" :style="{ width: toolbarWidth + 'px' }">
      <div class="right-toolbar-con">
        <el-button v-if="showToolButton('clearDesignerButton')" link type="primary" @click="clearFormWidget"> <svg-icon icon-class="el-delete" />{{ i18nt("designer.toolbar.clear") }}</el-button>
        <el-button v-if="showToolButton('previewFormButton')" link type="primary" @click="previewForm"> <svg-icon icon-class="el-view" />{{ i18nt("designer.toolbar.preview") }}</el-button>
        <el-button v-if="showToolButton('importJsonButton')" link type="primary" @click="importJson"> {{ i18nt("designer.toolbar.importJson") }}</el-button>
        <el-button v-if="showToolButton('exportJsonButton')" link type="primary" @click="exportJson"> {{ i18nt("designer.toolbar.exportJson") }}</el-button>
        <el-button v-if="showToolButton('exportCodeButton')" link type="primary" @click="exportCode"> {{ i18nt("designer.toolbar.exportCode") }}</el-button>
        <el-button v-if="showToolButton('generateSFCButton')" link type="primary" @click="generateSFC"> <svg-icon icon-class="vue-sfc" />{{ i18nt("designer.toolbar.generateSFC") }}</el-button>
      ...
      <el-dialog :title="i18nt('designer.toolbar.exportJson')" v-model="showExportJsonDialogFlag" :show-close="true" class="drag-dialog small-padding-dialog" center append-to-body :close-on-click-modal="false" :close-on-press-escape="false" :destroy-on-close="true">
        ...
        <template #footer>
          <div class="dialog-footer">
            ...
            <el-button @click="showExportJsonDialogFlag = false"> {{ i18nt("designer.hint.closePreview") }}</el-button>
          </div>
        </template>
      </el-dialog>
    </div>
      ...
      <el-dialog :title="i18nt('designer.toolbar.exportCode')" v-model="showExportCodeDialogFlag" :show-close="true" class="drag-dialog small-padding-dialog" center append-to-body width="65%" :close-on-click-modal="false" :close-on-press-escape="false" :destroy-on-close="true">
        ...
        <template #footer>
          <div class="dialog-footer">
            ...
            <el-button @click="showExportCodeDialogFlag = false"> {{ i18nt("designer.hint.closePreview") }}</el-button>
          </div>
        </template>
      </el-dialog>
    </div>
    ...
    <div v-if="showFormDataDialogFlag" class="" v-drag="['.nested-drag-dialog.el-dialog', '.nested-drag-dialog .el-dialog__header']">
      <el-dialog :title="i18nt('designer.hint.exportFormData')" v-model="showFormDataDialogFlag" :show-close="true" class="nested-drag-dialog dialog-title-light-bg" center :close-on-click-modal="false" :close-on-press-escape="false" :destroy-on-close="true" :append-to-body="true">
        ...
        <template #footer>
          <div class="dialog-footer">
            <el-button type="primary" class="copy-form-data-json-btn" :data-clipboard-text="formDataRawJson" @click="copyFormDataJson"> {{ i18nt("designer.hint.copyFormData") }}</el-button>
            <el-button @click="saveFormData">{{ i18nt("designer.hint.saveFormData") }}</el-button>
            <el-button @click="showFormDataDialogFlag = false"> {{ i18nt("designer.hint.closePreview") }}</el-button>
          </div>
        </template>
      </el-dialog>
    </div>
      ...
      >
        ...
        <template #footer>
          <div class="dialog-footer">
            ...
            <el-button type="primary" class="copy-vue3-sfc-btn" :data-clipboard-text="sfcCodeV3" @click="copyV3SFC"> {{ i18nt("designer.hint.copyVue3SFC") }}</el-button>
            ...
            <el-button @click="showExportSFCDialogFlag = false"> {{ i18nt("designer.hint.closePreview") }}</el-button>
          </div>
        </template>
      </el-dialog>
    </div>
...
<script>
import VFormRender from "@/components/form-designer/form-render/index.vue";
import CodeEditor from "@/components/form-designer/code-editor/index";
import Clipboard from "clipboard";
import { deepClone, copyToClipboard, generateId, getQueryParam, traverseAllWidgets, addWindowResizeHandler } from "@/components/form-designer/utils/util";
import i18n from "@/components/form-designer/utils/i18n";
import { generateCode } from "@/components/form-designer/utils/code-generator";
import { genSFC } from "@/components/form-designer/utils/sfc-generator";
import loadBeautifier from "@/components/form-designer/utils/beautifierLoader";
import { saveAs } from "file-saver";
import axios from "axios";
import SvgIcon from "@/components/form-designer/svg-icon/index";
...
export default {
  ...
  methods: {
    ...
    testSetFormJson() {
      let newFormJson = {
        widgetList: [
          { type: "static-text", icon: "static-text", formItemFlag: false, options: { name: "statictext111193", columnWidth: "200px", hidden: false, textContent: "多列表单", customClass: [], onCreated: "", onMounted: "", label: "static-text" }, id: "statictext111193" },
          { type: "divider", icon: "divider", formItemFlag: false, options: { name: "divider102346", label: "", columnWidth: "200px", direction: "horizontal", contentPosition: "center", hidden: false, customClass: [], onCreated: "", onMounted: "" }, id: "divider102346" },
          {
            type: "grid",
            category: "container",
            icon: "grid",
            cols: [
              {
                ...
                options: { name: "gridCol25469", hidden: false, span: 12, offset: 0, push: 0, pull: 0, responsive: false, md: 12, sm: 12, xs: 12, customClass: [] },
                id: "grid-col-25469"
              },
              {
                type: "grid-col",
                ...
                options: { name: "gridCol25125", hidden: false, span: 12, offset: 0, push: 0, pull: 0, responsive: false, md: 12, sm: 12, xs: 12, customClass: "" },
                id: "grid-col-25125"
              },
              {
                type: "grid-col",
                ...
                options: { name: "gridCol44470", hidden: false, span: 24, offset: 0, push: 0, pull: 0, responsive: false, md: 12, sm: 12, xs: 12, customClass: "" },
                id: "grid-col-44470"
              },
              {
                type: "grid-col",
                ...
                options: { name: "gridCol98223", hidden: false, span: 24, offset: 0, push: 0, pull: 0, responsive: false, md: 12, sm: 12, xs: 12, customClass: "" },
                id: "grid-col-98223"
              }
            ],
          ...
          { type: "divider", icon: "divider", formItemFlag: false, options: { name: "divider69240", label: "", columnWidth: "200px", direction: "horizontal", contentPosition: "center", hidden: false, customClass: "", onCreated: "", onMounted: "" }, id: "divider69240" },
          {
            type: "grid",
            category: "container",
            icon: "grid",
            cols: [
              {
                ...
                options: { name: "gridCol47242", hidden: false, span: 12, offset: 0, push: 0, pull: 0, responsive: false, md: 12, sm: 12, xs: 12, customClass: "" },
                id: "grid-col-47242"
              },
              {
                type: "grid-col",
                ...
                options: { name: "gridCol27970", hidden: false, span: 12, offset: 0, push: 0, pull: 0, responsive: false, md: 12, sm: 12, xs: 12, customClass: "" },
                id: "grid-col-27970"
              },
              {
                type: "grid-col",
                ...
                options: { name: "gridCol74653", hidden: false, span: 24, offset: 0, push: 0, pull: 0, responsive: false, md: 12, sm: 12, xs: 12, customClass: "" },
                id: "grid-col-74653"
              },
              {
                type: "grid-col",
                ...
                options: { name: "gridCol63781", hidden: false, span: 24, offset: 0, push: 0, pull: 0, responsive: false, md: 12, sm: 12, xs: 12, customClass: "" },
                id: "grid-col-63781"
              }
            ],
          ...
          { type: "divider", icon: "divider", formItemFlag: false, options: { name: "divider75887", label: "", columnWidth: "200px", direction: "horizontal", contentPosition: "center", hidden: false, customClass: [], onCreated: "", onMounted: "" }, id: "divider75887" },
          {
            type: "grid",
            category: "container",
            icon: "grid",
            cols: [
              {
                ...
                options: { name: "gridCol109912", hidden: false, span: 24, offset: 0, push: 0, pull: 0, responsive: false, md: 12, sm: 12, xs: 12, customClass: "" },
                id: "grid-col-109912"
              },
              {
                type: "grid-col",
                ...
                options: { name: "gridCol114653", hidden: false, span: 24, offset: 0, push: 0, pull: 0, responsive: false, md: 12, sm: 12, xs: 12, customClass: "" },
                id: "grid-col-114653"
              },
              {
                type: "grid-col",
                ...
                options: { name: "gridCol80867", hidden: false, span: 24, offset: 0, push: 0, pull: 0, responsive: false, md: 12, sm: 12, xs: 12, customClass: "" },
                id: "grid-col-80867"
              }
            ],
